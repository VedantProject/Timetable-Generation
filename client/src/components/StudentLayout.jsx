...
import { CalendarDays, LogOut, GraduationCap } from 'lucide-react';
...
const StudentLayout = () => {
  ...
  const navItems = [
    { name: 'Class Schedule', path: '/student', icon: <CalendarDays size={20} />, exact: true }
  ];
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <div className="w-64 bg-slate-900 border-r border-slate-800 shadow-xl flex flex-col justify-between transition-all duration-300">
        <div>
          <div className="h-20 flex items-center justify-center border-b border-slate-800 bg-slate-950/50">
            <h1 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-400 tracking-wider">
               STUDENT PORTAL
            </h1>
          </div>
        ...
        <div className="p-4 bg-slate-950/30 border-t border-slate-800">
          <div className="flex items-center mb-4 px-2">
            ...
            <div className="ml-3 overflow-hidden">
              ...
              <p className="text-xs text-amber-300 truncate">Student ID: {user?.email.split('@')[0]}</p>
            </div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50/50 relative">
        <div className="absolute top-0 left-0 w-full h-96 bg-gradient-to-b from-amber-100/50 to-transparent -z-10 pointer-events-none"></div>
        <div className="p-8">
          <Outlet />
        </div>
      </div>
